...
"""
BRR - Encode and decode SNES audio samples.

Converts between 16-bit mono WAV audio and the SNES BRR (Bit Rate Reduction)
format used by the S-SMP/DSP. Encoding selects the best filter (0-3) and shift
(0-12) per 16-sample block; decoding reconstructs PCM at 32 kHz.

Usage:
  r65x brr encode sample.wav -o sample.brr
  r65x brr decode sample.brr -o sample.wav
"""
...
def register_parser(subparsers):
    """Register the brr subcommand with the r65x CLI."""
    parser = subparsers.add_parser(
        'brr',
        help='Encode and decode SNES audio samples',
        description='Encode and decode SNES audio samples (BRR format).',
    )
    parser.add_argument('action', metavar='encode|decode',
                        help="Action type")
    parser.add_argument('input', metavar='inputfile',
                        help="Input file (.wav for encode, .brr for decode)")
    parser.add_argument('-o', '--output', required=True, metavar='outfile',
                        default=None, help="File path to output")


def brr_command(args):
    """Execute the brr command."""
    if not args.action or args.action not in ['encode', 'decode']:
        print("Error: action must be 'encode' or 'decode'", file=sys.stderr)
        sys.exit(1)

    if args.action == "encode" and not args.input.endswith(".wav"):
        print("Error: input file must be a wav", file=sys.stderr)
        sys.exit(1)

    if args.action == "decode" and not args.input.endswith(".brr"):
        print("Error: input file must be a brr", file=sys.stderr)
        sys.exit(1)
